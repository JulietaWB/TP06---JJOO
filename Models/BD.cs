...
public static class BD{
    ...
    ListarDeportistasxDeporte(int idDeporte)
    {
        string sql = "SELECT * FROM Deportistas WHERE IdDeporte= @p";
        List<Pais> ListaDeportistas = new List<Pais>(); 
        using (SqlConnection db =  new SqlConnection (connectionString)) 
        {
            ListaDeportistas= db.Query<Pais>(sql, new {p = idDeporte}).ToList();
        }
        return ListaDeportistas;
    }

    ListarDeportistasxPais(int idPais)
    {
        string sql = "SELECT * FROM Deportistas WHERE IdPais= @p";
        List<Pais> ListaDeportistas = new List<Pais>(); 
        using (SqlConnection db =  new SqlConnection (connectionString)) 
        {
            ListaDeportistas= db.Query<Pais>(sql, new {p = idPais}).ToList();
        }
        return ListaDeportistas;
    }
